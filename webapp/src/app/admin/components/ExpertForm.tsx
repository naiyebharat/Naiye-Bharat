...
import { X, CheckCircle2, AlertCircle, Image } from "lucide-react";
...
export default function ExpertForm({ onSuccess, onClose }: { onSuccess: () => void; onClose: () => void }) {
    ...
    onSubmit: async (values, { resetForm, setSubmitting }) => {
      try {
        ...
        if (response.ok) {
          ...
          resetForm();
          onSuccess();
          // Auto-close form after 1 second
          setTimeout(() => {
            onClose();
          }, 1000);
        } else {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4 border-b border-slate-100 dark:border-slate-800 pb-3 sticky top-0 bg-white dark:bg-[#0b1329] z-10">
          <div className="flex items-center gap-2">
            <span className="text-emerald-600 dark:text-[#00c2a8] text-xl font-bold">👤+</span>
            <h2 className="text-md font-bold text-slate-800 dark:text-white">Add New Expert</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-3">
          ...
          >
            <div>
              ...
              <input autoComplete="off" type="number" {...formik.getFieldProps('age')} className="w-full bg-slate-50/50 dark:bg-[#050b1d] border border-slate-200 dark:border-slate-800/80 rounded-lg px-3 py-1.5 text-xs text-slate-800 dark:text-slate-100 focus:outline-none focus:border-emerald-500 dark:focus:border-[#00c2a8] focus:ring-2 focus:ring-emerald-500/10 dark:focus:ring-0 transition-all duration-200" />
            ...
            <div>
              ...
              <select {...formik.getFieldProps('specialty')} className="w-full bg-slate-50/50 dark:bg-[#050b1d] border border-slate-200 dark:border-slate-800/80 rounded-lg px-3 py-1.5 text-xs text-slate-800 dark:text-slate-100 focus:outline-none focus:border-emerald-500 dark:focus:border-[#00c2a8] focus:ring-2 focus:ring-emerald-500/10 dark:focus:ring-0 transition-all duration-200 font-semibold">
          ...
          >
            ...
            <input autoComplete="off" type="text" {...formik.getFieldProps('language')} placeholder="English, Hindi" className="w-full bg-slate-50/50 dark:bg-[#050b1d] border border-slate-200 dark:border-slate-800/80 rounded-lg px-3 py-1.5 text-xs text-slate-800 dark:text-slate-100 focus:outline-none focus:border-emerald-500 dark:focus:border-[#00c2a8] focus:ring-2 focus:ring-emerald-500/10 dark:focus:ring-0 transition-all duration-200" />
          ...
          {/* Photo Link (Previously Video URL) */}
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <label className="block text-[10px] font-extrabold text-slate-700 dark:text-slate-300 tracking-wider mb-1 flex items-center gap-1.5">
              ...
              Instagram Profile Photo Link
            </label>
            <input type="url" {...formik.getFieldProps('videoUrl')} placeholder="https://instagram.com/..." className="w-full bg-slate-50/50 dark:bg-[#050b1d] border border-slate-200 dark:border-slate-800/80 rounded-lg px-3 py-1.5 text-xs text-slate-800 dark:text-slate-100 focus:outline-none focus:border-emerald-500 dark:focus:border-[#00c2a8] focus:ring-2 focus:ring-emerald-500/10 dark:focus:ring-0 transition-all duration-200" />
