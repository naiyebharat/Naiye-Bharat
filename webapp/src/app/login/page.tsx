...
export default function Login() {
  const [formType, setFormType] = useState<'login' | 'signup' | 'otp' | 'forgot'>('login');
  ...
  const handleBypass = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => {
      alert("Professional Access: You are currently entering as a verified guest.");
        ...
        <div className="container relative z-10 flex justify-center">
            <div className="w-full max-w-[450px] bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] overflow-hidden transition-all duration-500 hover:shadow-gold-500/10">
                ...
                <div className="p-10 pb-6 text-center">
                    <Link href="/">
                        <div className="w-24 h-24 mx-auto mb-6 hover:scale-110 transition-transform cursor-pointer">
                            <Image src="/img/logo.png" alt="NaiyeBharat Logo" width={96} height={96} className="w-full h-full object-contain" />
                        </div>
                    </Link>
                    ...
                    <p className="text-slate-500 mt-2 text-sm font-light">
                        {formType === 'login' && 'Enter your credentials to access the legal portal.'}
                        {formType === 'signup' && 'Join NaiyeBharat for secure legal collaboration.'}
                        {formType === 'otp' && 'Enter the code sent to your registered mobile.'}
                        {formType === 'forgot' && 'Provide your email to recover your access.'}
                    </p>
                </div>
                ...
                <form onSubmit={handleBypass} className="px-10 pb-10 space-y-5">
                    {formType === 'signup' && (
                        <div className="relative group">
                            <label className="text-[10px] uppercase tracking-widest text-slate-400 font-bold ml-1 mb-1 block">Full Name</label>
                            <input 
                                type="text" 
                    ...
                    <div className="relative group">
                        <label className="text-[10px] uppercase tracking-widest text-slate-400 font-bold ml-1 mb-1 block">Work Email</label>
                        <input 
                            type="email" 
                    ...
                    {(formType === 'login' || formType === 'signup') && (
                        <div className="relative group">
                             <div className="flex justify-between items-center mb-1">
                                <label className="text-[10px] uppercase tracking-widest text-slate-400 font-bold ml-1 block">Security Password</label>
                                {formType === 'login' && (
                                    <button type="button" onClick={() => setFormType('forgot')} className="text-[10px] uppercase tracking-widest text-gold-600 font-bold hover:text-oxford-900 transition-colors">Forgot?</button>
                                )}
                             </div>
                            <input 
                                type="password" 
                    ...
                    {formType === 'login' && (
                        <div className="relative py-4 flex items-center justify-center">
                            ...
                            <span className="relative px-4 bg-white text-[10px] uppercase tracking-widest text-slate-400 font-bold">Or secure access via</span>
                        </div>
                    )}

                    {formType === 'login' && (
                        <div className="flex gap-4">
                            <button type="button" className="flex-1 py-3 border border-slate-100 rounded-xl hover:bg-slate-50 transition-colors flex items-center justify-center gap-2">
                ...
                <div className="p-6 bg-slate-50 border-t border-slate-100 text-center">
                    {formType === 'login' ? (
                        <p className="text-sm text-slate-500">
                            Authorized personnel only. <button onClick={() => setFormType('signup')} className="text-gold-600 font-bold hover:underline">Request Access</button>
                        </p>
                    ) : (
                        <button onClick={() => setFormType('login')} className="text-sm text-gold-600 font-bold hover:underline tracking-wide uppercase text-xs">Back to Secure Login</button>
                    )}
                </div>
                ...
                <div className="pb-8 text-center flex items-center justify-center gap-2 opacity-30 grayscale hover:grayscale-0 hover:opacity-100 transition-all cursor-default">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest">End-to-End SSL Encrypted</span>
                </div>
            </div>
        </div>
    </main>
